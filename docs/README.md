...
> **Scope note.** ClassMate is **video-only** in this version — enforced at every layer (the upload presign rejects non-`video/*`, the content schema allows only `category=media`, and a DB CHECK constraint enforces it). The retrieval corpus is **transcript chunks only**; there is no PDF/slides/notes ingestion.
...
| # | Doc | What it covers |
|---|---|---|
| 1 | **[Data model](./data.md)** | The spine. Entities, ownership/cascade rules, the `content_chunks` dual index, idempotency & artifact-lineage patterns, and the schema-evolution story (incl. the video-only pivot). Read first — the others link back here for table shapes. |
...
| 4 | **[Streaming UX](./streaming-ux.md)** | The delivery layer. The SSE event contract (status → thinking → citations → answer → done), the turn state machine, the typewriter, dual finalization, citation deep-links, and the live→persisted handoff. |
...
- **Models** — three providers, tiered per task: Anthropic (router/answers), Gemini (chapters/summaries), OpenAI (embeddings). See [`rag-and-ai.md` §5](./rag-and-ai.md#5-model-orchestration-one-job-one-tier).
