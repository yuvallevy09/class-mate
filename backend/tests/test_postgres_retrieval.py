...
async def test_postgres_retrieve_filters_by_category(monkeypatch) -> None:
    ...
    try:
        async with SessionLocal() as session:
            ...
            # Sanity: tsv is generated by DB; ensure it exists for at least one row.
            chk = (await session.execute(select(ContentChunk).where(ContentChunk.content_id == content_exam.id))).scalar_one()
            assert chk.tsv  # computed column should be non-empty for non-empty text
